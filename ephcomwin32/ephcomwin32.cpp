// ephcomwin32.cpp : Defines the exported functions for the DLL application.
//


#include "ephcomwin32.h"
...
extern "C" EPHCOMWIN32_API int getnumobjects()
{
	return EPHCOM_NUMOBJECTS;
}
...
extern "C" EPHCOMWIN32_API int calcposvel(char* ephfile, double unixtime, /*out*/ double* result) {
	...
	*/
	if (testjd >= header1.ss[0] && testjd <= header1.ss[1]) {
		...
		if (ephcom_get_coords(infp, &header1, &coords, datablock) == 0) {
			memcpy(result, coords.et2, (EPHCOM_NUMOBJECTS * 6 + 2) * sizeof(double));
			fclose(infp);
			return 0;
		}
		else {
